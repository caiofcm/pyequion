...
# def pyequion_api():
    ...
    if endpoint == 'App.startup':
       ...
       # back_to_dict = json.loads(as_json_str)
        ...
        sys_eq_deslrd = pyequion.utils_api.create_eq_sys_from_serialized(sys_eq_serialized)
        solResult = solve_equilibrium(sys_eq_deslrd, concentrations, temperature, extraParameter, allowPrecipitation, nonidealityType)
        eq_sol_out = {
            'result': solResult.to_dict()
        }
...
def solve_equilibrium(sys_eq, concentrations, temperature, extraParameter, allowPrecipitation, nonidealityType):
    """
    Output: {
        'reactions': list of reactions in the system
    }
    ...
    fugacity_calc = 'pr' if sys_eq.closing_equation_type == pyequion.ClosingEquationType.OPEN else 'ideal'
