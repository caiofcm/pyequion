...
def afi(T):
    ...
    # CFCM - PitzerNaCl
    ds = -0.0004*T + 1.1188
    eer = 305.7 * np.exp(-np.exp(-12.741 + 0.01875*T) - T/219.0) #Zemaitis pg 644
    Aphi = 1.0/3.0 *(2.0*np.pi*Na*ds/1000)**0.5*(ee/(eer*k*T)**0.5)**3.0
...
def funb(ionic, beta, zc, za):
    ...
    if zc == 2 and za == -2:
        ...
        par3 = beta[1]*(-2*(1-(1+x1+0.5*x1*x1)*np.exp(-x1)))/(x1**2)
        par4 = beta[2]*(-2*(1-(1+x2+0.5*x2*x2)*np.exp(-x2)))/(x2**2)
    else:
...
def funphi(zc, zcc, ionic, a, theta):
    ...
    w = np.array([1.925154014814667, -0.060076477753119, -0.029779077456514,
    ...
    b = np.array([0.628023320520852, -0.028796057604906, 0.006519840398744,
                  -0.000242107641309, -0.000004583768938, 0.000000216991779,
                  -0.000000006944757, 0.462762985338493, 0.150044637187895,
                  -0.036552745910311, -0.001668087945272, 0.001130378079086,
                  -0.000887171310131, 0.000087294451594, 0.000034682122751,
                  -0.000003548684306, -0.000000250453880, 0.000000080779570,
                  0.000000004558555, -0.000000002849257, 0.000000000237816])
...
def loggammax(mc, ma, mn, zc, za, beta, cfi, thetac, thetaa, psiXac, psiXcc, lambdanX, t, ia, skip_ternary=False):
    ...
    termo6 = 0.0
    for i in range(0, len(mc)):
        termo6 += mc[i]*(2*bcx[i] + zg*ccx[i]) #ma[i]*(2.0*bma[i] + zg*cma[i])
    # termo6 = 0
    # for i in range(0, len(mc)):
    #     for j in range(0, len(ma)):
...
def activitywater(mc, ma, mn, zc, za, beta, cfi, thetac, thetaa, lambdanc, lambdana, t):
    ...
    soma = np.sum(m)
    osmotic = (2/soma)*(termo1 + termo2 + termo3 + termo4 + termo5 + termo6) + 1
