...
reactionsListVapourPhase = [
    {'CO2': 1, 'CO2(g)': -1, 'type': 'henry', 'log_K25': -1.468, 'log_K_coefs': ''}, #FIXME,  but logK(T) is hardcoded for CO2(g)
    # {
    #     "CO2-": 1.0,
    #     "CO2(g)": -1.0,
    #     "log_K25": "",
...
def create_equilibrium_from_reaction_and_species(reactions, species,
    known_tags, element_mass_balance, feed_compounds,
    initial_feed_mass_balance, closing_equation_type, return_intermediaries=False,
    ...
    "Species"
    engine_species = create_list_of_species_engine(species) #, species_activity_db)

    "Fill with Conductivity Parameters"
    fill_conductivity_in_species_from_db(engine_species, database_files['species'])

    engine_idxs = create_Indexes_instance(species, len(known_tags))

    "Mass Balances"
    mb_list_engine = create_list_of_mass_balances_engine(species, element_mass_balance,
        engine_idxs, feed_compounds, closing_equation_type,
        initial_feed_mass_balance, fixed_elements) #FIXME: remove mass balance detection logic to another function
    ...
    known_specie = obtain_know_specie_list_from_feed(initial_feed_mass_balance, engine_idxs.idx, reacs_irrev, feed_compounds)

    "Fixed Specie"
    if fixed_elements is not None:
        known_specie_constant = get_fixed_specie_as_known_specie(fixed_elements, engine_idxs.idx, feed_compounds) #FIXME
        if len(known_specie_constant) > 0:
            known_specie += known_specie_constant


    "Reactions"
    engine_reactions = create_list_of_reactions_engine(reactions, engine_idxs.idx)

    if closing_equation_type == ClosingEquationType.CARBON_TOTAL:
        known_specie = adjust_known_specie_list_when_closed_system(
            species, engine_idxs.idx,
            known_specie
        )
    ...
    if not dic_tuple: #len(dic_tuple) == 0:
        dic_tuple = [(-1, np.nan)] #DIC reference -> Remember Carbone total is just a mass balance, FIXME
    ...
    if possible_solid_reactions_in is None:
        reactionsListPreciptationDB = utils.load_from_db(database_files['phases'])
    else:
    ...
    if solid_reactions_in is None or solid_reactions_in == []:
        ...
        solid_reactions_in_ = numbafy_list_dict_str_float_reaction_storage(solid_reactions_in)

    sys_eq = core.EquilibriumSystem(
        engine_species, engine_idxs,
        engine_reactions, mb_list_engine, known_specie,
        dic_tuple, solid_reactions_, num_of_feeds, #activity_modeltype removed
        feed_compounds_List,
    ...
    if return_intermediaries:
        return sys_eq, species, reactions, engine_idxs.idx, \
...
def get_initial_comp_and_known_tags_from_ini_config(
        feed_compounds, initial_feed_mass_balance, closing_equation_type, fixed_elements=None):
...
def adjust_known_specie_list_when_closed_system(species, dict_indexes_species_conv, known_specie):
    tags_coefs_carbone_total = get_species_tags_with_an_element(species, 'C')
    species_indexes_carbone_total, species_coefs_carbone_total = get_idx_coef_from_species_tag_list(tags_coefs_carbone_total, dict_indexes_species_conv)
    known_specie_carbone_total_RHS = [core.MassBalance(
...
def get_dic_tuple(species, dict_indexes_species_conv):
    tags_coefs_carbone_total = get_species_tags_with_an_element(species, 'C')
    tags_coefs_carbone_total_no_solid = {key: val
        for key, val in tags_coefs_carbone_total.items()
        if '(s)' not in key
    }
    species_indexes_carbone_total, species_coefs_carbone_total = get_idx_coef_from_species_tag_list(
        tags_coefs_carbone_total_no_solid, dict_indexes_species_conv
    )
    dic_tuple = [(i, coef) for i, coef in zip(species_indexes_carbone_total, species_coefs_carbone_total)]
    return dic_tuple
...
def get_reacs_irrev_list(reactions, species):
    ...
    for i, reaction in enumerate(reactions):
        if reaction['type'] == 'irrev':
            _, reacs = utils.get_tags_of_prods_reacts(reaction) #Irreversivible alaways considering the prod
...
def obtain_know_specie_list_from_feed(initial_feed_mass_balance, dict_indexes_species_conv, reacs_irrev, feed_compounds):
    if initial_feed_mass_balance:
        # ini_tag_mb_conv = convert_species_tag_for_engine(initial_feed_mass_balance)
        ini_known_idxs_mass_list = [dict_indexes_species_conv[tag] for tag in initial_feed_mass_balance]
...
def walk_in_species_reactions(c, species, reactions, reactionsList):
    for r in reactionsList:
        ...
        if c in prods and are_others_in_side_of_reaction_kown(prods, species) and r['type'] != 'irrev': #IRREV ALWAYS SHOULD BE react -> prod
            reactions += [r]
            for in_element in reacs:
                if 'phase_name' in r and '(s)' in in_element:
                    ...
                    tag_add = in_element
                species.add(tag_add)
                walk_in_species_reactions(in_element, species, reactions, reactionsList)
...
def create_list_of_reactions_engine(reactions_list, dict_of_indexes_species):
    ...
    for reac in reactions_list:
        ...
        if not reac['log_K_coefs']:
            ...
            log_coefs[0:len(reac['log_K_coefs'])] = np.array(reac['log_K_coefs'])
...
def create_list_of_mass_balances_engine(list_species, element_list,
    idx_control, feed_list, closing_equation_type, initial_feed_mass_balance, fixed_elements):
    """
    ...
    if not element_list:
        if closing_equation_type in [ClosingEquationType.NONE, ClosingEquationType.CARBON_TOTAL, ClosingEquationType.OPEN]: #CARBON_TOTAL STAYS OR NOT?
    ...
    if initial_feed_mass_balance:
        ini_ele_no_signal = [re.sub(RX_NO_SIGNAL, '', tag) for tag in initial_feed_mass_balance]
        [ele_set.discard(item) for item in ini_ele_no_signal]
    if fixed_elements:
        ini_ele_no_signal = [re.sub(RX_NO_SIGNAL, '', tag) for tag in fixed_elements]
        [ele_set.discard(item) for item in ini_ele_no_signal]
    ...
    mass_balances = []
    for el in ele_set:
        tags_coefs, species_indexes = get_species_indexes_matching_element(list_species, el, idx_control.idx)
...
def get_species_indexes_from_tags_coefs(dict_of_indexes_species, tags_coefs):
    species_indexes = np.array([
        dict_of_indexes_species[replace_strings_for_engine_compatibility(tag)]
        for tag in tags_coefs
    ])
    return species_indexes

def get_reactions_species(reac):
    return [k for k in reac.keys() if utils.check_validity_specie_tag_in_reaction_dict(k)]

def get_elements_and_their_coefs(list_of_species):
    # Removing signals
    tags_no_signals = [re.sub(RX_NO_SIGNAL, '', tag) for tag in list_of_species]
...
def get_species_tags_with_an_element(list_of_species, element):
    ...
    try:
        elements.remove('size') #FIXME FIXME PUT SIZE IN THE IDX CONTROL CLASS, NOT IN IDX DICT
    except ValueError:
        pass
...
def get_indexes_feed_for_mass_balance(feeds: list, element: str):
    ...
    idxs_in_feed = [ i
        for i, feed in enumerate(feeds) if feed in tags_with_coefs
    ]
...
def get_guess_from_ideal_solution(reaction_system, args, jac=None, activities_db_file_name=None):
    x_guess0 = np.full(reaction_system.idx_control.idx['size'], -1e-3)
    _, fsol = solve_equilibrium_existing_activity_model(reaction_system, x_guess0, args,
        jac, TypeActivityCalculation.IDEAL, activities_db_file_name)
    x_guess = fsol.x
    return x_guess

def solve_equilibrium_existing_activity_model(reaction_system, x_guess, args=None,
        jac=None,
    ...
    if activity_model_type == TypeActivityCalculation.DEBYE:
        ...
        setup_log_gamma_func = core.act.setup_log_gamma_bdot_mean_activity_neutral
        calc_log_gamma = core.act.calc_log_gamma_dh_bdot_mean_activity_neutral

    setup_log_gamma(reaction_system, args[1], args[0],
        setup_log_gamma_func, activities_db_file_name) #args: TK, c_feed
...
def conv_reaction_engine_to_db_like(reac_solid_precip_no_polymorph):
    ...
    for reac in reac_solid_precip_no_polymorph:
        d = {tag.split('__')[0]: coef for tag, coef in zip(reac.species_tags, reac.stoic_coefs)}
...
def match_indexes_of_a_solution_with_another(dict_idx_map_helper, dict_indexes_species_conv, sys_eq, solution_no_precipitation, x_guess):
    for tag_no_precip, val in dict_idx_map_helper.items():
        if val > -1:
            if dict_indexes_species_conv[tag_no_precip] >= sys_eq.idx.size:
                continue
            x_guess[dict_indexes_species_conv[tag_no_precip]] = \
...
def convert_as_latex_specie(reac, r):
    ...
    ss = '{}{{{}^{{{}{}}}}} +'.format(coefstr, tag_no_signal, charge_str, signal)
    return ss
...
def get_list_of_reactions_latex(initial_comp, allow_precipitation=False,
    database_files=DEFAULT_DB_FILES):
...
def tag_to_element_coef(tag):
    ...
    if len(dupls) > 0:
        dupl_cases = [[v_c for v_c in elements_with_coefs if v_c[0] in dupl] for dupl in dupls]
        summed_duplicated = [reduce(lambda cum, t_c: (t_c[0], cum[1]+t_c[1]), dupl) for dupl in dupl_cases]
        elements_coef_comb = [v_c for v_c in elements_with_coefs if v_c[0] not in dupls]
