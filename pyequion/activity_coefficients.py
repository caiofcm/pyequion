# from pyequion.properties_utils import *
...
def calc_log_gamma_dh_bdot(idx_ctrl, species, I: float, T: float): #FIXME: idx_ctrl to reaction_sys ?
    ...
    for sp in species:
        if np.isfinite(sp.p_scalar['dh_a']):
            ...
            a = sp.p_scalar['dh_a']
            b = sp.p_scalar['dh_b']
            logg = -A*sp.z**2*np.sqrt(I)/(1+B*a*np.sqrt(I))+b*I
...
# @numba.njit WITH PR
def calc_log_gamma_dh_bdot_with_pengrobinson(idx_ctrl, species, I: float, T: float): #FIXME: idx_ctrl to reaction_sys ?
...
def setup_log_gamma_bdot_mean_activity_neutral(reaction_sys,
    T, db_species, c_feed=None):
    ...
    for sp in reaction_sys.species:
        if sp.z != 0:
            ...
            reac_match = utils.get_dissociating_ions(sp.name, reaction_sys.reactions)
            if reac_match is None:
                ...
                sp.d['mean_coefs'] = {tag: stoic for tag, stoic in zip(tags, stoics)}
...
def setup_log_gamma_pitzer(reaction_sys,
    T, db_species, c_feed=None):
    ...
    for i, sp_i in enumerate(cations): #FIXME: symmetric matrix, take advantage of it and the db can have less information
        try:
