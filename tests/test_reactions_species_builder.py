...
#--------------------------------------------
EXAMPLE_SPECIES = ['H2O', 'NaHCO3', 'CaCl2', 'H+', 'OH-', 'Na+', 'NaOH', 'HCO3-', 'CO2', 'CO2(g)', 'CO3--', 'NaCO3-', 'Na2CO3', 'Ca++', 'CaHCO3+', 'CaCO3', 'CaOH+', 'Cl-']
EXAMPLE_REACTIONS = [
...
def test_nahco3_open():
    ...
    E_SPECIES = ['H2O', 'NaHCO3', 'H+', 'OH-', 'Na+', 'NaOH', 'HCO3-', 'CO2', 'CO2(g)', 'CO3--', 'NaCO3-', 'Na2CO3']
    E_REACTIONS = [{'H+': -1, 'H2O': 1, 'OH-': -1, 'id_db': 10, 'type': 'rev'}, {'HCO3-': -1, 'Na+': -1, 'NaHCO3': 1, 'id_db': 2, 'type': 'rev'}, {'H+': 1, 'H2O': -1, 'Na+': -1, 'NaOH': 1, 'id_db': 0, 'type': 'rev'}, {'CO3--': -1, 'H+': -1, 'HCO3-': 1, 'id_db': 9, 'type': 'rev'}, {'CO2': -1, 'H+': 1, 'HCO3-': 1, 'id_db': 8, 'type': 'rev'}, {'CO2': 1, 'CO2(g)': -1, 'id_db': 7, 'type': 'henry'}, {'CO3--': -1, 'Na+': -1, 'NaCO3-': 1, 'id_db': 1, 'type': 'rev'}, {'CO3--': -1, 'Na+': -2, 'Na2CO3': 1, 'id_db': 3, 'type': 'rev'}]
...
def test_cacl2_open():
    ...
    E_SPECIES = ['H2O', 'CaCl2', 'CO2(g)', 'H+', 'OH-', 'Ca++', 'CaOH+', 'Cl-', 'CO2', 'HCO3-', 'CO3--', 'CaCO3', 'CaHCO3+']
    E_REACTIONS = [{'H+': -1, 'H2O': 1, 'OH-': -1, 'id_db': 10, 'type': 'rev'}, {'Ca++': -1, 'CaCl2': 1, 'Cl-': -2, 'id_db': -1, 'type': 'irrev'}, {'Ca++': -1, 'CaOH+': 1, 'H+': 1, 'H2O': -1, 'id_db': 4, 'type': 'rev'}, {'CO2': 1, 'CO2(g)': -1, 'id_db': 7, 'type': 'henry'}, {'CO2': -1, 'H+': 1, 'HCO3-': 1, 'id_db': 8, 'type': 'rev'}, {'CO3--': -1, 'H+': -1, 'HCO3-': 1, 'id_db': 9, 'type': 'rev'}, {'CO3--': -1, 'Ca++': -1, 'CaCO3': 1, 'id_db': 12, 'type': 'rev'}, {'Ca++': -1, 'CaHCO3+': 1, 'HCO3-': -1, 'id_db': 11, 'type': 'rev'}]
    species, reactions = rbuilder.get_species_reactions_from_compounds(initial_comp)
    for s in species:
        assert s in E_SPECIES
    assert len(E_REACTIONS) == len(reactions)
...
def test_identify_element_in_species_list():
    tags_list = {'H+', 'OH-', 'CO2', 'CO3--', 'HCO3-',
                            'Na+', 'NaOH', 'NaCO3-', 'NaHCO3', 'Na2CO3',
                            'CaOH+', 'CaHCO3+', 'CaCO3', 'Ca++', 'Cl-', 'H2O'}
...
def test_engine_cacl2_solve_open_0p01():
    ...
    solution = solve_equilibrium(sys_eq, x_guess=x_guess, args=args,
        # jac=nahco3_residual_jacobian
    )
...
def test_engine_with_parenthesis_Mn():
    ...
    assert np.isclose(meanMnCO3, solution.gamma[sys_eq.idx_control.idx['Mn++']], 1e-2)
    assert_solution_result(solution, EXPECTED)
    assert compare_with_expected_perc_tuple(solution.saturation_index['Pyrochroite'], EXPECTED['SI']['Pyrochroite'])
    assert compare_with_expected_perc_tuple(solution.saturation_index['Rhodochrosite'], EXPECTED['SI']['Rhodochrosite'])
...
def test_engine_mix_closed_add_IS():
    ...
    pyequion.print_solution(solution)
    assert_solution_result(solution, EXPECTED)
    assert compare_with_expected_perc_tuple(solution.saturation_index['Halite'], EXPECTED['SI']['Halite'])
    assert compare_with_expected_perc_tuple(solution.saturation_index['Calcite'], EXPECTED['SI']['Calcite'])
    assert compare_with_expected_perc_tuple(solution.saturation_index['Aragonite'], EXPECTED['SI']['Aragonite'])
...
def test_engine_mix_closed_add_IS_dic_modified():
    ...
    pyequion.print_solution(solution)
    assert_solution_result(solution, EXPECTED)
    assert compare_with_expected_perc_tuple(solution.saturation_index['Calcite'], EXPECTED['SI'])
...
def test_engine_mix_default_feed_mb_closed():
    ...
    sys_eq = create_equilibrium(feed_compounds,
        closing_equation_type, element_mass_balance=None,
        initial_feed_mass_balance=initial_feed_mass_balance)
...
def test_engine_baco3_solve_1mM_open():
    ...
    pyequion.print_solution(solution)
    assert_solution_result(solution, EXPECTED)
    assert compare_with_expected_perc_tuple(solution.saturation_index['Witherite'], EXPECTED['SI'])
...
def test_engine_water_co2_solve_fixed_pH_closing_equation():
    ...
    solution, fsol, _ = solve_equilibrium(sys_eq, args=args, x_guess=x_guess, ret_fsol=True)
    print(fsol.message)
    assert_solution_result(solution, EXPECTED)
    pass
...
def test_engine_mix_solve_1_and_5_closed_precipitation():
    ...
    args = (np.array([cCaCO3, cCaCl2]), TK, carbone_total) #Instead of pCO2->DIC
...
def test_engine_activity_external_ideal():
    ...
    solution = solve_equilibrium(sys_eq, x_guess=x_guess,
        args=args,
...
def test_engine_caco3_cacl2_pitzer():
    ...
    assert np.isclose(9.327, solution.pH, 1e-2) #9.45 is D.H, check for Pitzer with phreeqc
...
def test_converted_signals_plus_minus():
    comps = ['H2O', 'NaHCO3', 'CaCl2', 'H+', 'OH-', 'Na+', 'NaOH', 'HCO3-', 'CO2', 'CO2(g)', 'CO3--', 'NaCO3-', 'Na2CO3', 'Ca++', 'CaHCO3+', 'CaCO3', 'CaOH+', 'Cl-']
...
def test_feed_indexes_for_mb():
    ...
    idxs_coefs_feed = rbuilder.get_indexes_feed_for_mass_balance(feed_compounds, element)
