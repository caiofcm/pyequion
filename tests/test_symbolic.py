...
def test_string_inputs_as_tuple_descontruction():
    s = string_inputs_as_tuple_descontruction(x)
...
def test_lambdastr_as_regular_function_v_simple_fun_numpy(symbolic_vars):
    v = sympy.symbols('v')
    expr = v**2 - 4*v
    s = string_lambdastr_as_function(expr, v, use_numpy=True)
...
def test_lambdastr_as_regular_function_numpy_imports(symbolic_vars):
    expr, y, a = symbolic_vars
    s = string_lambdastr_as_function(expr, y, a, use_numpy=True, include_imports=True)
    assert 'def calc_expression(y, a):\n' in s
    assert STR_AUX_TEST_NUMPY in s
    assert 'import numpy' in s

def test_save_to_file_stringfied_function_expression(data_files_dir, symbolic_vars):
    expr, y, a = symbolic_vars
    s = string_lambdastr_as_function(expr, y)
    save_function_string_to_file(s, str(data_files_dir.join('aow.py')))

def test_save_to_file_stringfied_function_expression_save_local(data_files_dir, symbolic_vars):
    expr, y, a = symbolic_vars
    s = string_lambdastr_as_function(expr, y)
    save_function_string_to_file(s)
...
def test_numbafied_string_function(string_function_sample):
    ...
    new_ss = numbafy_function_string(ss)
...
def test_numbafied_string_function_kwarg(string_function_sample):
    ...
    new_ss = numbafy_function_string(ss, kwarg_string)

    assert 'import numba' in new_ss
    assert '@numba.njit(cache=True)\ndef cacl2_residual_jacobian(x, args, )' in new_ss

def test_regex_function_defined(string_function_sample):
    ss = string_function_sample
    r = regex_function_defined(ss)
...
def test_numbafied_string_dummy_args_added(string_function_sample):
    ...
    kwarg_string = 'cache=True'
    add_arg_fun = 'dummy'
    new_ss = numbafy_function_string(ss, kwarg_string, add_arg_fun)

    assert 'import numba' in new_ss
    assert '@numba.njit(cache=True)\ndef cacl2_residual_jacobian(x, args, {})'.format(add_arg_fun) in new_ss
