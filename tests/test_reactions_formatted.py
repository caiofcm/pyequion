...
def test_formatted_single():
    ...
    reacs_formatted = format_reaction_list_as_latex_mhchem(reac)
    assert type(reacs_formatted) == list
    assert '\ce' in reacs_formatted[0]
...
def test_formatted_from_reaction_storage():
    reac_listing = [{'H+': 1.0, 'H2O': -1.0, 'OH-': 1.0}, {'H+': 1.0, 'H2O': -1.0, 'Na+': -1.0, 'NaOH': 1.0}, {'Cl-': 1.0, 'Na+': 1.0, 'NaCl': -1.0, 'irrev': 1.0}]
    reacs_formatted = format_reaction_list_as_latex_mhchem(reac_listing)
    assert True

def test_formatted_as_itemize_function():
    initial_comp = ['H2O', 'NaHCO3', 'CaCl2']
    itemized = get_itemized_latex_from_compounds(initial_comp)
    print(itemized)
    assert type(itemized) == str
    assert '\\begin{itemize}' in itemized
