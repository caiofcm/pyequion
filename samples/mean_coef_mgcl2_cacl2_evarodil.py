...
def main_cacl2_meancoeff():
    ...
    solutions = {
        key: [
                pyequion.solve_solution( {'CaCl2': c*1e3}, sys_eq,
                activity_model_type=key)
                for c in cIn_span
            ]
        for key in methods
    }
    gamma_means = {key: np.array([pyequion.get_mean_activity_coeff(sol, 'CaCl2') for sol in solutions[key]])
        for key in methods
    }
...
def cacl2_mgcl2_nacl_mean_activity_coeffs():
    ...
    gamma_means_CaCl2 = {key: np.array([pyequion.get_mean_activity_coeff(sol, 'CaCl2') for sol in solutions[key]])
        for key in methods
    }
    I_CaCl2 = {key: np.array([sol.I for sol in solutions[key]]) for key in methods}
    ...
    solutions = {
        key: [
                pyequion.solve_solution( {tag: c*1e3}, sys_eq,
                activity_model_type=key)
                for c in cIn_span
            ]
        for key in methods
    }
    gamma_means_MgCl2 = {key: np.array([pyequion.get_mean_activity_coeff(sol, tag) for sol in solutions[key]])
        for key in methods
    }
    I_MgCl2 = {key: np.array([sol.I for sol in solutions[key]]) for key in methods}
    ...
    gamma_means_NaCl = {key: np.array([pyequion.get_mean_activity_coeff(sol, tag) for sol in solutions[key]])
        for key in methods
    }
    I_NaCl = {key: np.array([sol.I for sol in solutions[key]]) for key in methods}
    ...
    for key in methods:
        plt.plot(I_CaCl2[key], gamma_means_CaCl2[key], plot_opts[key], label=key)
    plt.xlabel('I [M]', fontsize=12)
    plt.title('$\gamma_{CaCl_2}$', fontsize=fs)
    # plt.ylabel('$\gamma_{\pm}$')
    plt.legend(fontsize=12)
    plt.subplot(1,3,2)
    plt.plot(I_exp_cnvMgCl2, data_exp_raw_collected_mgcl2[:,1], 'sk', label='exp')
    plt.xlabel('I [M]', fontsize=fs)
    plt.title('$\gamma_{MgCl_2}$', fontsize=fs)
    # plt.ylabel('$\gamma_{\pm}$')
    for key in methods:
        plt.plot(I_MgCl2[key], gamma_means_MgCl2[key], plot_opts[key], label=key)
    plt.legend(fontsize=12)
    plt.subplot(1,3,3)
    plt.plot(I_exp_cnvNaCl, data_exp_raw_collected_NaCl[:,1], 'sk', label='exp')
    plt.xlabel('I [M]', fontsize=fs)
